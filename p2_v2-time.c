...
main( int argc, char *argv[] ) {
    ...
    if( argc != 4 ) {
        if( myid == 0 )
          fprintf( stderr, "Usage: %s I  N_INNER  N_OUTER\n", argv[0] );
        M6378_Finalize();
        exit(1);
    }
    ...
    if( !uold || !unew ) {
      ...
      if (unew) free(unew);
      if (myid == 0) {
        if(utotal) free(utotal);
      }
      M6378_Finalize();
      exit(1);
    }

    // Initialize uold array
    if (myid == 0) {
    ...
    for(k = 0; k < N_OUTER; k++) {
      ...
      // Update boundary values of unew
      unew[0] = uold[0];
      unew[I] = uold[I];
      ...
      if (myid < (np-1)) {
          ...
          R = ((urr - 2*uold[I] + uold[I-1])*odx2) - f(xstar);
